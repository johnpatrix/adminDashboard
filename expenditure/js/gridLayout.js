...
function submitInputedData()
{
	...
	var bill_type=document.getElementById('bill_Type_name').value;	
	if(batchId !="" && bill_type!= "0" )
	{
			...
			if(dirtyBit ==0){
				$.post( "insertBill.php", {'userData': dataset,'batchId':batchId,'bill_type':bill_type},function(data){window.location.href='pdfGenerate.php';});
			}
			else if(dirtyBit==1){
				$.post( "insertBill.php", {'userData': dataset,'batchId':batchId,'bill_type':bill_type},function(data){window.location.href='pdfGenerate.php?dirty=1';});
			}
	}
	else
	alert("You have missed selecting one or both the Option");	
}
